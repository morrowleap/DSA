...
public:
  ...
  {
    long long oddIndicesCount = n / 2;
    long long evenIndicesCount = n - oddIndicesCount;
    return (myPow(5, evenIndicesCount) * myPow(4, oddIndicesCount)) % MOD;
  }
};

int main()
{
  cout << "Problem: Count Good Numbers\n";
  cout << "Description: A digit string is good if the digits at even indices "
          "are even, "
          "and the digits at odd indices are prime (2, 3, 5, or 7). "
          "Given an integer n, return the total number of good digit strings "
          "of length n.\n";
  cout << "Input: Enter the number of elements (n): ";
